...
- **`v1/`** — Current version. Same crypto as v0, but uses **contract storage** instead of calldata/events for retrieval.
  ...
  - Data is mutable (re-storing for the same `dataId` overwrites).
  - From `v1/`: `npm run store:encrypt`, `npm run retrieve:decrypt`, `npm run test`.
  - See `v1/README.md` for the full motivation and delta from v0.
